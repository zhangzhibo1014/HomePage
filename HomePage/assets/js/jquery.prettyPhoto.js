(function(a) {
  ...
  a.fn.prettyPhoto = function(y) {
    ...
    if (y.keyboard_shortcuts) {
      a(document)
        ...
        .bind("keydown.prettyphoto", function(C) {
    ...
    a.prettyPhoto.initialize = function() {
      ...
      rel_index = isSet
        ? set_position
        : a("a[" + settings.hook + "^='" + theRel + "']").index(a(this));
      f(this);
    ...
    a.prettyPhoto.open = function(C) {
      ...
      if (settings.hideflash) {
        a("object,embed,iframe[src*=youtube],iframe[src*=vimeo]").css(
          "visibility",
          "hidden"
        );
      }
      ...
      movie_width = parseFloat(d("width", pp_images[set_position]))
        ? d("width", pp_images[set_position])
        : settings.default_width.toString();
      movie_height = parseFloat(d("height", pp_images[set_position]))
        ? d("height", pp_images[set_position])
        : settings.default_height.toString();
      r = false;
      if (movie_height.indexOf("%") != -1) {
        movie_height = parseFloat(
          (a(window).height() * parseFloat(movie_height)) / 100 - 150
        );
        r = true;
      }
      if (movie_width.indexOf("%") != -1) {
        movie_width = parseFloat(
          (a(window).width() * parseFloat(movie_width)) / 100 - 150
        );
        r = true;
      }
      $pp_pic_holder.fadeIn(function() {
        ...
        switch (l(pp_images[set_position])) {
          ...
          case "youtube":
            ...
            d("rel", pp_images[set_position])
              ? (movie += "?rel=" + d("rel", pp_images[set_position]))
              : (movie += "?rel=1");
            if (settings.autoplay) {
              movie += "&autoplay=1";
            }
            toInject = settings.iframe_markup
              .replace(/{width}/g, w.width)
              .replace(/{height}/g, w.height)
          ...
          case "custom":
            ...
            break;
          case "inline":
            myClone = a(pp_images[set_position])
              .clone()
              .append('<br clear="all" />')
    ...
    a.prettyPhoto.changeGalleryPage = function(C) {
      ...
      slide_speed =
        C == "next" || C == "previous" ? settings.animation_speed : 0;
    ...
    a.prettyPhoto.close = function() {
      ...
      a("div.pp_pic_holder,div.ppt,.pp_fade").fadeOut(
    ...
    function p() {
      ...
      $pp_pic_holder
        .find(".pp_content")
        .animate(
          { height: w.contentHeight, width: w.contentWidth },
          settings.animation_speed
        );
      $pp_pic_holder.animate(
        {
          top: projectedTop,
          left:
            B / 2 - w.containerWidth / 2 < 0 ? 0 : B / 2 - w.containerWidth / 2,
    ...
    function n() {
      if (
        ...
        l(pp_images[set_position]) == "image"
      ) {
        itemWidth = 52 + 5;
        navWidth =
        ...
        itemsPerPage = Math.floor(
          (w.containerWidth - 100 - navWidth) / itemWidth
        );
        itemsPerPage =
    ...
    if (!pp_alreadyInitialized && c()) {
      ...
      setTimeout(function() {
        a("a[" + y.hook + "^='" + hashRel + "']:eq(" + hashIndex + ")").trigger(
          "click"
        );
      }, 50);
    }
  ...
  function c() {
    var f = location.href;
    hashtag =
      f.indexOf("#prettyPhoto") !== -1
        ? decodeURI(f.substring(f.indexOf("#prettyPhoto") + 1, f.length))
        : false;
    if (hashtag) {
